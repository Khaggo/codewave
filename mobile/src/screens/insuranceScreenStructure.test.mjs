...
test('insurance shell uses a header-triggered vehicle picker instead of a body vehicle card', () => {
  const source = read('./InsuranceInquiryScreen.js')
  const styleSource = read('./insuranceInquiryStylesA.js')
  const shellSource = read('./insurance/InsuranceModeShell.js')

  assert.match(source, /import styles from '\.\/insuranceInquiryStyles'/)
  assert.match(styleSource, /import \{ Platform, StyleSheet \} from 'react-native'/)
  ...
  assert.match(styleSource, /sheetList:\s*\{[\s\S]*?maxHeight:\s*\d+/)
...
test('request and documents tabs use the redesigned premium layouts', () => {
  ...
  const requestFlowSource = read('./insurance/insuranceRequestFlow.mjs')

  assert.match(requestSource, /title="Request"/)
  assert.match(requestSource, /INSURANCE_REQUEST_STAGES/)
  assert.match(requestFlowSource, /Reason & Coverage/)
  assert.match(requestSource, /Claim request/)
  assert.match(requestSource, /Request details/)
  assert.match(requestSource, /Incident date and time/)
  assert.match(requestSource, /Incident location/)
  assert.match(requestSource, /Insurance provider/)
  assert.match(requestSource, /Additional details/)
  assert.match(requestSource, /Required documents/)
  assert.match(requestSource, /Optional documents/)
  assert.match(requestSource, /title="Review"/)
  assert.match(requestSource, /Submit request/)
  assert.match(requestSource, /accessibilityLiveRegion="assertive"/)
  assert.match(requestSource, /DateTimePicker/)
...
test('request and documents tabs reflect the real insurance workflow from intake notes', () => {
  ...
  assert.match(requestSource, /What do you need\?/)
  assert.match(requestFlowSource, /Documents & Review/)
  ...
  assert.match(screenSource, /getInsuranceRequirements/)
  assert.match(screenSource, /buildAuthoritativeRequirementsChecklist/)
