...
test('critical staff booking, job-order, QA, and invoice screens use business-readable references', () => {
  ...
  const jobOrderViewModel = read('frontend/src/screens/jobOrderWorkbenchViewModel.mjs')
  const qaAudit = read('frontend/src/screens/QAAuditWorkspace.js')
  const qaAuditPresentation = read('frontend/src/screens/QAAuditPresentation.jsx')
  ...
  assert.match(jobOrderViewModel, /function formatBookingReference\(record\) \{/)
  assert.match(jobOrderViewModel, /function formatJobOrderReference\(record\) \{/)
  ...
  assert.match(qaAuditPresentation, /function formatJobOrderReference\(jobOrder\) \{/)
  assert.match(qaAuditPresentation, /function getLoadedJobOrderReference\(jobOrderId, jobOrderOptions, qualityGate = null\) \{/)
