...
export default function AccessoryStockWorkspace() {
  ...
  return (
    <main className="mx-auto max-w-[1500px] space-y-5 p-4 lg:p-6">
      ...
      <form onSubmit={submit} className="grid gap-3 border-y border-surface-border py-4 md:grid-cols-[minmax(240px,1fr)_150px_minmax(280px,1.3fr)_auto]">
        <select className="input-field" aria-label="Accessory variant" value={adjustment.variantId} onChange={(event) => setAdjustment((current) => ({ ...current, variantId: event.target.value }))} required><option value="">Select variant</option>{state.items.map((row) => <option key={row.variant.id} value={row.variant.id}>{row.product.name} - {row.variant.name}</option>)}</select>
        <input className="input-field" aria-label="Signed stock adjustment" type="number" placeholder="+10 or -2" value={adjustment.quantityDelta} onChange={(event) => setAdjustment((current) => ({ ...current, quantityDelta: event.target.value }))} required />
        <input className="input-field" aria-label="Stock adjustment reason" placeholder="Required audit reason" minLength={3} value={adjustment.reason} onChange={(event) => setAdjustment((current) => ({ ...current, reason: event.target.value }))} required />
        <button className="btn-primary min-h-11" disabled={action.busy}><Boxes size={15} /> Adjust</button>
      </form>
