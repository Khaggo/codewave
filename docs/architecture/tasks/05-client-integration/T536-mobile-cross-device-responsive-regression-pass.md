...
Make the mobile customer app stable across different phone sizes so service discovery, booking dates, booking cards, garage, insurance, and bottom navigation do not shift or overflow when the user tests on another device.

## Deliverables

- responsive audit of `mobile/src/screens/Dashboard.js`
- responsive audit of shared mobile components used by booking, garage, insurance, and profile surfaces
...
- Tightened booking service, vehicle, date, insurance, and home dashboard cards with `minWidth: 0`, wrapping, compact variants, and non-shrinking icons/actions where needed.
...
- Small phones under `360px`: booking date cards stack full-width, quick actions become a two-column grid, and bottom-nav insets shrink.
- Baseline phones around `390px`: service cards, slot definitions, booking date cards, and insurance rows keep two-column or wrapped layouts without horizontal scrolling.
- Larger phones at `430px+`: booking availability uses three stable date-card columns while garage and insurance content retain readable widths.
...
- changing backend booking or insurance contracts
