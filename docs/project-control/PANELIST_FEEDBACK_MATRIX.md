...
| Billing | Billing details not fully functional. | 3, 6 | QA Passed / Monitor | Service billing, ecommerce billing, manual payment, and booking-to-cash now pass in the latest 20/20 whole-system Playwright rerun. Residual polish remains around invoice lookup specificity, immediate just-paid invoice visibility, and completed-history refresh clarity. |
| Admin Data | Data entries: shop, catalog, service. | 3 | QA Passed / Monitor | Staff catalog/service/admin billing and customer Shop catalog/checkout/rewards now pass in the latest 20/20 whole-system Playwright rerun. Keep runtime-created product proof and service-pricing proof in the panel evidence pack. |
...
| IDs | Vehicle ID and transaction ID should not use hash data; generate own ID. | 2, 3 | QA Passed / Monitor | Booking references are persisted unique business IDs, ecommerce order/invoice references use `ORD-...` / `INV-...`, mobile Shop product detail prefers product code, Back-Jobs lineage labels are readable, and the adviser-owned readable-ID sweep now passes on fresh listeners. Keep monitoring older/untested records, but the latest blocker bundle did not reopen this issue. |
